...
//    VendorClient client;
    ...
    public RegisterVendorResponseBean registerVendor(RegisterVendorRequestBean registerVendorRequestBean) {
        VendorEntity vendorEntity=vendorDao.insertVendor(new VendorEntity(registerVendorRequestBean.getVendorOutletName(), registerVendorRequestBean.getUserId(), registerVendorRequestBean.getState(), registerVendorRequestBean.getCity(), registerVendorRequestBean.getPinCode(), registerVendorRequestBean.getOpeningTime(), registerVendorRequestBean.getClosingTime(), registerVendorRequestBean.getNoOfStaff()));
        List<VendorServiceMappingEntity> vendorServiceMappingEntities=registerVendorRequestBean.getServices().stream().map(key->new VendorServiceMappingEntity(new ServiceEntity(key.getServiceId()),vendorEntity,key.getPrice(),key.getNoOfSeats())).collect(Collectors.toList());
    ...
    @Override
    public VendorEntity fetchVendor(String vendorId) {
